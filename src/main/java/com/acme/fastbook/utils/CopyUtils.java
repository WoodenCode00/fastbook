...
public class CopyUtils {
	...
	/**
	 * Copies properties from source object to target object for
	 * non-null values of source object. Method leverages Spring's
	 * {@link BeanUtils#copyProperties(Object, Object, String...)} method.
	 * <p><p>
	 * Note, this method will update {@code target} input
	 * parameter.
	 * 
	...
	/**
	 * Gets an array of property names of {@code source} parameter which have NULL values
	 *  
	 * @param source
	 * @return
	...
	private static String[] getPropertyNamesWithNullValues(final Object source) throws IntrospectionException {
		...
		PropertyDescriptor[] propertyDescriptors = Introspector
				  .getBeanInfo(source.getClass(), Object.class)
				  .getPropertyDescriptors();

		// Since methods in lambda throw checked exceptions, we use a custom ThrowingPredicate to convert them to RuntimeExceptions  
		ThrowingPredicate<PropertyDescriptor> isPropertyNullPredicate = 
				propertyDescriptor -> 
		            (propertyDescriptor.getReadMethod() != null && propertyDescriptor.getReadMethod().invoke(source) == null);

		List<String> nullProperties = Arrays.stream(propertyDescriptors)
		    .filter(isPropertyNullPredicate)
