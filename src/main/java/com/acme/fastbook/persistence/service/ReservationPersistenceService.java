...
public interface ReservationPersistenceService {
  ...
  /**
   * Checks if reservation dates are available for the corresponding BookingItem
   * ID and writes new/updated reservation to DB. Reservations with statuses from
   * the supplied list of {@code excludedStatuses} are excluded from the search
   * logic.
   * 
   * @param reservation      {@link Reservation} object to write/update in DB
   * @param excludedStatuses List of excluded statuses
   * 
   * @return Reservation wrote to DB. It might be different from the original
   *         reservation object.
   */
  Reservation checkDatesAndCreate(Reservation reservation, List<ReservationStatus> excludedStatuses);

  /**
   * Checks if reservation dates are available for the Reservation being updated
   * and updates existing reservation in DB. Reservations with statuses from the
   * supplied list of {@code excludedStatuses} are excluded from the search logic.
   * 
   * @param reservation      {@link Reservation} object to write/update in DB.
   * @param excludedStatuses List of excluded statuses
   * 
  ...
  /**
   * Finds all active reservations for provided Booking Item ID and range of
   * dates. Reservations which are partially covered by provided range of dates
   * will also be included in result. Status of the Reservation should not be in
   * the list of {@code excludedStatuses}.
   * 
   ...
   */
  List<Reservation> findAllForBookingItemIdAndWithinDateRange(UUID bookingItemId, ZonedDateTime startRange,
